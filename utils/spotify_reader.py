...
class Spotify_train:
    ...
    def __init__(
        ...
        for fullpath in tqdm(train_fullpaths, total=len(train_fullpaths)):
            f = open(fullpath)
            js = f.read()
            f.close()
        ...
        total_trk_uri_list, trk_count_list, track_uri2id = self.create_uri2id(
            o_dict, trk_min_count, 0
        )
        del o_dict
        ...
        total_art_uri_list, art_count_list, artist_uri2id = self.create_uri2id(
            o_dict, art_min_count, len(track_uri2id)
        )
        del o_dict

        # Get the Cumulative Distribution Function of the tracks and division points
        ...
        print(
            ...
            % (
                ...
                len(self.playlist_artists),
                len(self.playlist_titles),
            )
        )
        for tracks_uri, artists_uri, title in tqdm(
            zip(self.playlist_tracks, self.playlist_artists, self.playlist_titles),
            ...
            playlists.append([tracks_id, artists_id, title])
        ...
        print("train")
        with open(save_dir + "/" + "train.json", "w") as make_file:
            json.dump(file_data, make_file, indent="\t")
        print(
            "num playlists: %d, tracks_total: %d, tracks>=min_count: %d, artists>=min_count: %d"
            % (
                self.num_playlists,
                len(total_trk_uri_list),
                len(track_uri2id),
                len(artist_uri2id),
            )
        )
    ...
    def create_uri2id(o_dict, min_count, start_from):
        ...
        uri2id = dict(
            zip(valid_uri_list, range(start_from, start_from + len(valid_uri_list)))
        )
...
class Spotify_test:
    ...
    def __init__(
        self, test_fullpaths, train_json, test_seeds_num, save_dir, is_shuffle
    ):
        ...
        for fullpath in tqdm(test_fullpaths):
            f = open(fullpath)
            js = f.read()
            f.close()
            mpd_slice = json.loads(js)
        ...
        if self.is_shuffle:
            name = name + "r"
        print(name)
        with open(save_dir + "/" + name + ".json", "w") as make_file:
            json.dump(file_data, make_file, indent="\t")
    ...
    def process_playlist_for_test(self, playlist):
        ...
        for track, artist in zip(
            tracks[: self.test_seeds_num], artists[: self.test_seeds_num]
        ):
            if track != -1:
        ...
        self.num_playlists += 1
        self.playlists.append([seeds_tracks, seeds_artists, name, answers])

    @staticmethod
    def get_class(class_divpnt, track):
        if track < class_divpnt[0]:
            return 0
        elif track >= class_divpnt[0] and track < class_divpnt[1]:
            return 1
        elif track >= class_divpnt[1] and track < class_divpnt[2]:
            return 2
        elif track >= class_divpnt[2]:
            return 3


class Spotify_challenge:
    def __init__(
        self, challenge_fullpaths, train_json, save_dir, num_trk_lst, in_order
    ):
        with open(train_json) as data_file:
            train = json.load(data_file)
        self.track_uri2id = train["track_uri2id"]
