...
if __name__ == "__main__":
    ...
    args.add_argument(
        ...
        type=str,
        default="./data",
        help="directory where the outputs are stored",
    )
    args.add_argument(
        "--mpd_tr", type=str, default="./mpd_train", help="train mpd path"
    )
    args.add_argument("--mpd_te", type=str, default="./mpd_test", help="test mpd path")
    args.add_argument(
        "--mincount_trk", type=int, default=5, help="minimum count of tracks"
    )
    args.add_argument(
        "--mincount_art", type=int, default=3, help="minimum count of artists"
    )
    args = args.parse_args()

    train_fullpaths = fullpaths_generator(args.mpd_tr)
    train_fold = Spotify_train(
        train_fullpaths=train_fullpaths,
        trk_min_count=args.mincount_trk,
        art_min_count=args.mincount_art,
        is_title_normalize=True,
